...
let cu_to_json_opt ?(exe="cu-to-json") (fname : string) : (Yojson.Basic.t, int * string) Result.t =
  ...
  | Unix.WEXITED n, Ok _ -> Error (n, "Expecting exit status 0, but got " ^ string_of_int n)
  | Unix.WEXITED n, Error e -> Error (n, "Error parsing output: " ^ e) 
  | _, Error e -> Error (1, e)
  | _, _ -> Error (1, "Unknown error")

let cu_to_json ?(exe="cu-to-json") (fname : string) : Yojson.Basic.t =
  match cu_to_json_opt ~exe:exe fname with
  | Ok x -> x
  | Error (r, m) ->
